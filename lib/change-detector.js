...
const fs = require('fs');
const { uploadDiff, isR2Configured } = require('./storage');
...
async function compareFrames(frameAPath, frameBPath, outputDiffName) {
  ...
  if (outputDiffName) {
    ...
    fs.writeFileSync(diffFullPath, diffBuf);

    if (isR2Configured) {
      uploadDiff(diffImagePath, diffBuf).catch(e =>
        console.error('[ChangeDetector] R2 diff upload error:', e.message)
      );
    }
  }
