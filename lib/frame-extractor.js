...
const fs = require('fs');
const { uploadFrame, isR2Configured } = require('./storage');
...
async function extractAndStore(videoPath, segment, pool) {
  try {
    const { framePath, fullPath } = await extractFrame(videoPath, segment.id);

    if (isR2Configured && fs.existsSync(fullPath)) {
      const buf = fs.readFileSync(fullPath);
      await uploadFrame(framePath, buf);
    }
